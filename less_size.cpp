...
void consumer_pack(Buffer *buffer,int id){
    ...
    while (fg){
        ...
        buffer->mtx.unlock();
        //if (thread_chro_num>=chro_number_all) { fg=0;}

        if (!fg) break;
        if ((iter = sam_itr_querys(idx, hdr, chronosome[thread_chro_num])) == 0) {continue;}
...
//    fout.open("./out_less.fq");
    ...
    chro_number_all=0;
    for (int i=0;i<24;i++){
        strcpy(chronosome[chro_number_all],chromo[i]);
        strcat(chronosome[chro_number_all++],":1-1000000");
        for (int j=2000000;j<=140000000;j+=1000000){
            strcpy(chronosome[chro_number_all],chromo[i]);
            strcat(chronosome[chro_number_all],":");
            strcat(chronosome[chro_number_all],to_string(j-1000000+1).c_str());
            strcat(chronosome[chro_number_all],"-");
            strcat(chronosome[chro_number_all++],to_string(j).c_str());
        }
        strcpy(chronosome[chro_number_all],chromo[i]);
        strcat(chronosome[chro_number_all++],":140000000-");
    }
    strcpy(chronosome[chro_number_all++],chromo[24]);
//    for (int i=0;i<chro_number_all;i++){
//        printf("%s\n",chronosome[i]);
//    }
...
/*
 * number into change is 6524934
change time is :     	1.920448	 sec
 6453929
 6583494
 */