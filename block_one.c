...
//            .next_in = (Bytef*)src,
//            .avail_in = slen,
//            .total_in = 2*BGZF_MAX_BLOCK_SIZE,
//            .next_out = (Bytef*)dst,
//            .avail_out = *dlen,
//            .total_out = 2*BGZF_MAX_BLOCK_SIZE
//    };
    z_stream zs = {
            .zalloc = NULL,
            .zfree = NULL,
...
static void *bgzf_decode_func(struct bam_block *comp,struct bam_block *un_comp) {
    un_comp->pos=0;
...
//    printf("block_len is %d\n",block_len);
//    printf("read block is no-32-4 %d\n",b->l_data - c->l_qname+c->l_qname);
//    printf("read block is no-4-exextranul %d\n",b->l_data - c->l_qname+c->l_qname+32-c->l_extranul);
//    printf("read block is no %d\n",b->l_data - c->l_qname+c->l_qname+32+4);
    if ((block_len+c->l_extranul)!=(b->l_data - c->l_qname+c->l_qname+32)){
        printf("diff is %d %d\n",block_len,b->l_data - c->l_qname+c->l_qname+32-c->l_extranul);
    }
...
int main(){
    TDEF(fq)
    TSTART(fq)
    Buffer=(char *)malloc(sizeof (char)*(Maxn+10000));
    samFile *sin;
    sam_hdr_t *hdr;
    bam1_t *b;
    ...
    int count=0;
    int size=0;
    struct bam_block *comp=(struct bam_block*)malloc(sizeof(bam_block));
    comp->errcode=0;comp->pos=0;
    comp->length=BGZF_MAX_BLOCK_SIZE;
    struct bam_block *un_comp=(struct bam_block*)malloc(sizeof(bam_block));
    un_comp->errcode=0;un_comp->pos=0;
    un_comp->length=BGZF_MAX_BLOCK_SIZE;
    int N=0,M=0;
    while (read_block(sin->fp.bgzf,comp) == 0) {
        count++;
        bgzf_decode_func(comp,un_comp);
        printf("N is %d\nM is %d\n",N,M);
        while(read_bam(un_comp,b,0)>=0){
            ...
            if (pos>Maxn-1)
            {
                //fwrite(Buffer,sizeof(char),pos,outStream);
                pos=0;
            }
            Buffer[pos++]='@';
