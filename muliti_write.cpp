...
const int write_buffer_all = 30;
int write_bg=0;
int write_ed=0;
const int n_thread=15;
int thread_have=n_thread;
const long long Maxn = 1e7;
char Buffer[n_thread][Maxn+10000];
char writeBuffer[write_buffer_all][Maxn+10000];
long long writepos[write_buffer_all]={0};
long long pos[n_thread];
long long num[n_thread]={0};
void write_pack(){
    while(chro_number<chro_number_all || write_bg!=write_ed || thread_have){
        std::this_thread::sleep_for(chrono::milliseconds(5));
        //cout << thread_have << endl;
        while (write_bg!=write_ed){
            fout.write(writeBuffer[write_bg],writepos[write_bg]);
            write_bg=(write_bg+1)%write_buffer_all;
        }
    }
}
...
void consumer_pack(int id){
    ...
    while (fg){
        ...
        while (sam_itr_next(hin, iter, b)>=0) {
            ...
            {
                mtx.lock();
                while ((write_ed+1)%write_buffer_all==write_bg){
                    std::this_thread::sleep_for(chrono::milliseconds(10));
                }
                memcpy(writeBuffer[write_ed],Buffer[id],pos[id]);
                writepos[write_ed]=pos[id];
                write_ed=(write_ed+1)%write_buffer_all;
                mtx.unlock();
                pos[id]=0;
            }
            Buffer[id][pos[id]++]='@';
...
int main(){
    ...
    Bam[n_thread]=new thread(&write_pack);
